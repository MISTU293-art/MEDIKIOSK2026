...
app.use((err, req, res, next) => {
  ...
  if (req.accepts('html')) {
    res.status(500).render('partials/error', {
      ...
      message: 'An unexpected system error occurred: ' + (err.message || 'Please try again'),
      user: null
    });
  } else {
  ...
  try {
    ...
    server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        ...
        logger.warn(`Port ${PORT} is occupied by another process. Automatically switching to fallback port ${fallbackPort}...`);
        PORT = fallbackPort;
