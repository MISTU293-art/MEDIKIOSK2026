...
let systemSettings = {
  ...
  custom_disclaimer:
    "AI-generated draft. Requires review and confirmation by an authorized healthcare professional.",
...
exports.getPatients = async (req, res) => {
  try {
    ...
    // NORMAL PATIENTS
    // ==========================================
    const normalPatients = await Patient.find({});

    const normalData = normalPatients.map((patient) => ({
      ...patient.toObject(),

      // Mark as normal patient
      patientType: "Normal",
      isEmergency: false,
    ...
    const emergencyData = emergencyPatients.map(
      (patient) => ({
        ...patient.toObject(),

        // Emergency schema uses name/mobile
        // instead of fullName/phone
        fullName: patient.name || "-",
        phone: patient.mobile || "-",

        // Important flag for EJS
        patientType: "Emergency",
        isEmergency: true,

        // Emergency-specific values
        priority: "Emergency",
        status: patient.status || "queued",
    ...
    const patients = allPatients.slice(
      startIndex,
      startIndex + pageSize
    );
    ...
    // ==========================================
    const stats = {
      total: normalData.length + emergencyData.length,
    ...
    res.render("admin/patients", {
      title: "Patient Directory & Export",
    ...
    res.status(500).send(
      "Error loading patient directory: " +
        err.message
    );
  }
};
  ...
  try {
    systemSettings = {
      ...
      inactivity_timeout_sec:
        parseInt(req.body.inactivity_timeout_sec, 10) || 120,
      ...
      enabled_languages: Array.isArray(req.body.enabled_languages)
        ? req.body.enabled_languages
        : req.body.enabled_languages
          ? [req.body.enabled_languages]
    ...
    logger.audit(
      "SUPERADMIN_SETTINGS_UPDATED",
      req.user ? req.user.email : "admin@hospital.org",
      {
...
exports.getReports = async (req, res) => {
  try {
    ...
    const priorityStats = {
      Emergency: patients.filter((p) => p.priority === "Emergency Red-Flag")
        .length,
      Urgent: patients.filter((p) => p.priority === "Urgent").length,
      Normal: patients.filter((p) => p.priority === "Normal" || !p.priority)
        .length,
    };
    const deptStats = {};
    patients.forEach((p) => {
...
exports.postOverridePriority = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to override priority." });
  }
};
...
exports.getExportIntakesCsv = async (req, res) => {
  try {
    const patients = await Patient.find({});
    let csv =
      "Token,UHID,Full Name,Age,Gender,Phone,Aadhaar,Department,Doctor,Room,Priority,Status,Created At\n";
    patients.forEach((p) => {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="medikiosk_ayush_intakes_export.csv"',
    );
    return res.send(csv);
  } catch (err) {
...
exports.getExportPatientsExcel = async (req, res) => {
  ...
  const rows = patients
    .map(
      (patient) =>
        `<tr><td>${patient.cardNumber || ""}</td><td>${patient.uhid}</td><td>${patient.fullName}</td><td>${patient.age}</td><td>${patient.gender}</td><td>${patient.phone}</td><td>${patient.department || ""}</td><td>${patient.status || ""}</td><td>${new Date(patient.createdAt).toISOString()}</td></tr>`,
    )
    .join("");
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="medikiosk-patients.xls"',
  );
